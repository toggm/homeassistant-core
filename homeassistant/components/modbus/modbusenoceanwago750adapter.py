...
class ModbusEnOceanWago750Adapter(ModbusEnoceanAdapter):
    ...
    def __init__(
        self, hub: ModbusHub, device_address: Any, input_address: int, output_address: int
    ) -> None:
    ...
    async def readNextBytesFromHub(self) -> bytearray:
        ...
        while status_byte_on == control_byte_on:
            ...
            control = await self._hub.async_pb_call(
                self._device_address, self._output_address, 2, CALL_TYPE_REGISTER_HOLDING
            )
            if control is None:
                return bytearray()
            ...
            status = await self._hub.async_pb_call(
                self._device_address, self._input_address, 2, CALL_TYPE_REGISTER_INPUT
            )
            if status is None:
                return bytearray()
