...
async def async_modbus_setup(
    ...
    async def async_stop_modbus(event: Event) -> None:
        """Stop Modbus service."""
        _LOGGER.debug("async_stop_modbus")
...
class ModbusUpdateListener:
    ...
    def notify(
        ...
        return self._func(
            result, self._device_address, self._input_type, self._min_address - offset
        )
...
class ModbusHub:
    ...
    async def async_close(self) -> None:
        ...
        if (
            DATA_ENOCEAN in self.hass.data
            and ENOCEAN_DONGLE in self.hass.data[DATA_ENOCEAN]
        ):
            dongle = self.hass.data[DATA_ENOCEAN][ENOCEAN_DONGLE]
            if isinstance(dongle, ModbusEnOceanDongle):
                dongle.unload()
